...
module Xcodeproj
	class Project
		...
		def self.create_project(path)
			...
			applicationTarget = project.new_target(:application, obj["name"], :ios, '9.2', nil, :swift)
			...
			buildSettingsConfig = {
			    ...
			    'PRODUCT_BUNDLE_IDENTIFIER' => 'com.sallen.$(PRODUCT_NAME)'
			}
			...
			buildSettingsConfig.each do |key, array|
      		...
      		project.save(path + "/" + obj["name"] + ".xcodeproj")      
			project
		end
	end
end
